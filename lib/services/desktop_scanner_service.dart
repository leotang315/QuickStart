...
class DesktopScannerService {
  ...
  /// 备份桌面文件
  Future<BackupInfo> backupDesktopItems(List<DesktopItem> items) async {
    ...
    for (final item in items) {
      try {
        ...
        if (originalFile.existsSync()) {
          // 备份文件
          await originalFile.copy(backupItemPath);
        } else if (originalDir.existsSync()) {
          // 备份文件夹（递归复制）
          await _copyDirectory(originalDir, Directory(backupItemPath));
        }
        ...
        print('备份文件失败: ${item.originalPath}, 错误: $e');
      }
    }
  ...
  /// 清理桌面文件
  Future<void> cleanDesktopItems(List<DesktopItem> items) async {
    for (final item in items) {
      try {
        final originalFile = File(item.originalPath);
        final originalDir = Directory(item.originalPath);
        ...
        print('删除桌面文件失败: ${item.originalPath}, 错误: $e');
      }
    }
  }
  
  /// 恢复桌面文件
  Future<void> restoreDesktopItems() async {
    ...
    for (final item in backupInfo.items) {
      try {
        ...
        if (backupFile.existsSync()) {
          // 恢复文件
          await backupFile.copy(item.originalPath);
        } else if (backupDir.existsSync()) {
          // 恢复文件夹
          await _copyDirectory(backupDir, Directory(item.originalPath));
        }
      } catch (e) {
        print('恢复文件失败: ${item.originalPath}, 错误: $e');
      }
    }
  }
